...
(() => {
  ...
  function buildTimeline() {
    ...
    if (datasets.length === 0) { setTimeout(buildTimeline, 200); return; }
    ...
    if (!scrollHint) {
      ...
      scrollHint.innerHTML = '<strong>Traverse</strong><span class="scroll-arrow">🔭</span>';
    ...
    if (typeof boostTouchScroll === "function") boostTouchScroll(modalCard);
    ...
    initVizKeyboardNav({
      modal: timelineModal,
      getNodes: function () {
        return _allSlivers.filter(function (s) {
          return !s.el.classList.contains("tl-hidden");
        }).map(function (s) {
          return { el: s.el, targetX: 0, targetY: s.startOff, _hidden: false };
        });
      },
      onActivate: function (n) { n.el.click(); },
      liveRegionId: "tl-live",
    });
  }
  ...
  function buildSliver(s) {
    ...
    el.setAttribute("aria-label", cleanName + (titleStr ? ", " + titleStr : "") + ", " + r.display);
